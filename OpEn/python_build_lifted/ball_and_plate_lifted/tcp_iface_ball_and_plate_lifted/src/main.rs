///
/// Auto-generated TCP server for optimizer: ball_and_plate_lifted
///
...
fn execution_handler(
    ...
    match initial_guess {
        ...
        Some(u0) => {
            if u0.len() != BALL_AND_PLATE_LIFTED_NUM_DECISION_VARIABLES {
    ...
    if let Some(y0) = &execution_parameter.initial_lagrange_multipliers {
        if y0.len() != BALL_AND_PLATE_LIFTED_N1 {
    ...
    if parameter.len() != BALL_AND_PLATE_LIFTED_NUM_PARAMETERS {
...
fn run_server(tcp_config: &TcpServerConfiguration) {
    ...
    let mut p = [0.0; BALL_AND_PLATE_LIFTED_NUM_PARAMETERS];
    let mut cache = initialize_solver();
    info!("Done");
    let listener = TcpListener::bind(format!("{}:{}", tcp_config.ip, tcp_config.port)).unwrap();
    let mut u = [0.0; BALL_AND_PLATE_LIFTED_NUM_DECISION_VARIABLES];
...
fn main() {
    let matches = App::new("OpEn TCP Server [ball_and_plate_lifted]")
        .version("0.0.0")
        .author("John Smith")
