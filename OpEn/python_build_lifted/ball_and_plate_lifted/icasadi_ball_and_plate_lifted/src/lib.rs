...
extern "C" {
    fn init_interface_ball_and_plate_lifted();
    fn cost_function_ball_and_plate_lifted(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double) -> c_int;
    fn grad_cost_function_ball_and_plate_lifted(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double)
        -> c_int;
    fn mapping_f1_function_ball_and_plate_lifted(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double,
    ) -> c_int;
    fn mapping_f2_function_ball_and_plate_lifted(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double,
    ) -> c_int;
    // Compute all preconditioning parameters
    fn preconditioning_www_ball_and_plate_lifted (
        arg: *const *const c_double,
    ) -> c_int;
    fn init_penalty_function_ball_and_plate_lifted(
        arg: *const *const c_double,
        ipnlt: *mut *mut c_double,
    ) -> c_int;
    fn get_w_cost_ball_and_plate_lifted() -> c_double;
} // END of extern C
...
pub fn get_w_cost() -> f64 {
    unsafe {
        get_w_cost_ball_and_plate_lifted() as f64
    }
}
...
///
pub fn init_ball_and_plate_lifted() {
    unsafe {
...
pub fn cost(u: &[f64], xi: &[f64], static_params: &[f64], cost_value: &mut f64) -> i32 {
    ...
    unsafe {
        cost_function_ball_and_plate_lifted(
            arguments.as_ptr(),
            cost.as_mut_ptr(),
        ) as i32
    }
...
pub fn grad(u: &[f64], xi: &[f64], static_params: &[f64], cost_jacobian: &mut [f64]) -> i32 {
    ...
    unsafe {
        grad_cost_function_ball_and_plate_lifted(
            arguments.as_ptr(),
            grad.as_mut_ptr()
        ) as i32
    }
...
pub fn mapping_f1(
    ...
    unsafe {
         mapping_f1_function_ball_and_plate_lifted(
            arguments.as_ptr(),
...
pub fn mapping_f2(
    ...
    unsafe {
         mapping_f2_function_ball_and_plate_lifted(
            arguments.as_ptr(),
            constraints.as_mut_ptr()
        ) as i32
    }
...
pub fn precondition(
    ...
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    unsafe {
         preconditioning_www_ball_and_plate_lifted(arguments.as_ptr()) as i32
    }
}
...
pub fn initial_penalty(
    ...
    // preconditioning_init_penalty_function_ball_and_plate_lifted
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    let ip = &mut [rho_init as *mut c_double];

    unsafe {
         init_penalty_function_ball_and_plate_lifted(
            arguments.as_ptr(),
            ip.as_mut_ptr(),
        ) as i32
    }
